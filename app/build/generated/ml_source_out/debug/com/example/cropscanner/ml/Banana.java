...
public final class Banana {
  ...
  private Banana(@NonNull Context context, @NonNull Model.Options options) throws IOException {
    model = Model.createModel(context, "banana.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static Banana newInstance(@NonNull Context context) throws IOException {
    return new Banana(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static Banana newInstance(@NonNull Context context, @NonNull Model.Options options) throws
      IOException {
    return new Banana(context, options);
  }
