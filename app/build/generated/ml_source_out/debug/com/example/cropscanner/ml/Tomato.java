...
public final class Tomato {
  @NonNull
  private final Model model;

  private Tomato(@NonNull Context context, @NonNull Model.Options options) throws IOException {
    model = Model.createModel(context, "tomato.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static Tomato newInstance(@NonNull Context context) throws IOException {
    return new Tomato(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static Tomato newInstance(@NonNull Context context, @NonNull Model.Options options) throws
      IOException {
    return new Tomato(context, options);
  }
