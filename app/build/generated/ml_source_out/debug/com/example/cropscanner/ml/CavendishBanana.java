package com.example.cropscanner.ml;
...
public final class CavendishBanana {
  @NonNull
  private final Model model;

  private CavendishBanana(@NonNull Context context, @NonNull Model.Options options) throws
      IOException {
    model = Model.createModel(context, "cavendish_banana.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static CavendishBanana newInstance(@NonNull Context context) throws IOException {
    return new CavendishBanana(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static CavendishBanana newInstance(@NonNull Context context,
      @NonNull Model.Options options) throws IOException {
    return new CavendishBanana(context, options);
  }
